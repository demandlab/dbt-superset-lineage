...
class Superset:
    ...
    def _headers(self, **headers):
        ...
        csrf_token_response = requests.request(
            'GET',
            f"{self.api_url}/security/csrf_token/",
            headers={'Authorization': f'Bearer {self.access_token}'},
            timeout=30,
        )
        csrf_token = csrf_token_response.json()['result']

        return {
            'X-CSRF-Token': csrf_token,
            'Cookie': csrf_token_response.headers.get('set-cookie', ''),
    ...
    def request(
        ...
        **request_kwargs
        ...
        Args:
            ...
            refresh_token_if_needed: Whether the ``access_token`` should be automatically
                refreshed if needed.
        ...
        Raises:
            HTTPError: There is an HTTP error (detected by
                ``requests.Response.raise_for_status``) even after retrying with a fresh
                ``access_token``.
        """
        logger.info(
            "About to %s execute request for endpoint %s", method, endpoint
        )
